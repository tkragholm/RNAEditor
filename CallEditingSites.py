...
class CallEditingSites(object):
    ...
    def printAttributes(self):
        ...
        Helper.info("\t intronDistance:" + str(self.rnaEdit.params.intronDistance), self.rnaEdit.logFile,
    ...
    def removeHomopolymers(self, variants, outFile, distance):
        startTime = Helper.getTime()
        Helper.info(" [%s] remove Missmatches from homopolymers " % (startTime.strftime("%c")), self.rnaEdit.logFile,
                    self.rnaEdit.textField)
        ...
        # output statistics
        Helper.info("\t\t %d out of %d passed the Homopolymer-Filter" % (numberPassed, mmNumberTotal),
                    self.rnaEdit.logFile, self.rnaEdit.textField)
        Helper.printTimeDiff(startTime, self.rnaEdit.logFile, self.rnaEdit.textField)
    ...
    def blatSearch(self, variants, outFile, minBaseQual, minMissmatch):
        startTime = Helper.getTime()
        Helper.info(" [%s] Search non uniquely mapped reads" % (startTime.strftime("%c")), self.rnaEdit.logFile,
                    self.rnaEdit.textField)
        ...
        if not os.path.isfile(tempFasta) or not os.path.getsize(
                tempFasta) > 0:  # check if temFast exists and is not empty. If it exist it will not be created again
            ...
            #########    CREATE FASTA FILE        #######
            #############################################
            Helper.info(" [%s] Create fasta file for blat " % (startTime.strftime("%c")), self.rnaEdit.logFile,
                        self.rnaEdit.textField)
            counter = 1
            ...
            for varKey in variants.variantDict.keys():
                ...
                if len(alignements) >= minMissmatch:
                    ...
                    for sequence in alignements:
                        tempFastaFile.write("> " + variant.chromosome + "-" + str(
                            variant.position) + "-" + variant.ref + "-" + variant.alt + "-" + str(
                            missmatchReadCount) + "\n" + sequence + "\n")
                ...
                if counter % 1000 == 0:
                    ...
                    Helper.info(str(counter) + " of " + str(mmNumberTotal) + " variants done", self.rnaEdit.logFile,
                                self.rnaEdit.textField)
                    sys.stdout.flush()
        ...
        if not os.path.isfile(pslFile) or not os.path.getsize(pslFile) > 0:
            cmd = [self.rnaEdit.params.sourceDir + "blat", "-stepSize=5", "-repMatch=2253", "-minScore=20",
                   "-minIdentity=0", "-noHead", self.rnaEdit.params.refGenome, tempFasta, pslFile]
            # print cmd
        ...
        Helper.info(" [%s] Parse Blat output to look for non uniquely mapped reads" % (startTime.strftime("%c")),
                    self.rnaEdit.logFile, self.rnaEdit.textField)
        ...
        if not os.path.isfile(outFile):
            ...
            for line in pslFile:  # summarize the blat hits
                ...
                try:
                    blatScore = [pslFields[0], pslFields[13], pslFields[17], pslFields[18],
                                 pslFields[20]]  # #of Matches, targetName, blockCount, blockSize, targetStarts
                except IndexError:
                    Helper.warning("Not enough Values in '%s' (Skip)" % line, self.rnaEdit.logFile,
                                   self.rnaEdit.textField)
                    continue
            ...
            siteDict = {}
            discardDict = {}
            Helper.info(" [%s] Analyse Blat hits (Slow)" % (startTime.strftime("%c")), self.rnaEdit.logFile,
                        self.rnaEdit.textField)
            ...
            ##############################################################################
            Helper.info(" [%s] Deleting invalid variants" % (startTime.strftime("%c")), self.rnaEdit.logFile,
                        self.rnaEdit.textField)
            ...
            Helper.info("\t\t %d Missmatches had fewer than %d missmatching-Reads." % (mmNumberTooSmall, minMissmatch),
                        self.rnaEdit.logFile, self.rnaEdit.textField)
            Helper.info(
                "\t\t %d Missmatches had more missaligned reads than correct ones." % mmReadsSmallerDiscardReads,
                self.rnaEdit.logFile, self.rnaEdit.textField)
    ...
    def startAnalysis(self):
        ...
        global variants, nonAluVariants
        '''check if result file already exists'''
        if os.path.isfile(
                self.rnaEdit.params.output + ".editingSites.clusters") and self.rnaEdit.params.overwrite == False:
            Helper.status("\t [SKIP] Final result file already exist", self.rnaEdit.logFile, self.rnaEdit.textField,
                          "green")
            return 1
        ...
        cmd = ["java", "-Xmx12G", "-jar", self.rnaEdit.params.sourceDir + "GATK/GenomeAnalysisTK.jar",
               ...
               "-D", self.rnaEdit.params.dbsnp, "-o", vcfFile, "-metrics", self.rnaEdit.params.output + ".snp.metrics",
               "-nt", self.rnaEdit.params.threads, "-l", "ERROR",
               "-stand_call_conf", self.rnaEdit.params.standCall, "-stand_emit_conf", self.rnaEdit.params.standEmit,
        ...
        if not os.path.isfile(self.rnaEdit.params.output + ".noSNPs.vcf") or self.rnaEdit.params.overwrite == True:
            ...
            if not os.path.isfile(self.rnaEdit.params.output + ".noReadEdges.vcf"):
                variants = VariantSet(self.rnaEdit.params.output + ".noSNPs.vcf", self.rnaEdit.logFile,
                                      self.rnaEdit.textField)
        ...
        if not os.path.isfile(self.rnaEdit.params.output + ".noReadEdges.vcf") or self.rnaEdit.params.overwrite == True:
            ...
            if not os.path.isfile(self.rnaEdit.params.output + ".alu.vcf") or not os.path.isfile(
                    self.rnaEdit.params.output + ".nonAlu.vcf"):
                variants = VariantSet(self.rnaEdit.params.output + ".noReadEdges.vcf", self.rnaEdit.logFile,
                                      self.rnaEdit.textField)
        ...
        if (not os.path.isfile(self.rnaEdit.params.output + ".alu.vcf") or not os.path.isfile(
                self.rnaEdit.params.output + ".nonAlu.vcf")) or self.rnaEdit.params.overwrite == True:
            ...
            aluVariants = VariantSet(self.rnaEdit.params.output + ".alu.vcf", self.rnaEdit.logFile,
                                     self.rnaEdit.textField)
            if not os.path.isfile(self.rnaEdit.params.output + ".noSpliceJunction.vcf"):
                nonAluVariants = VariantSet(self.rnaEdit.params.output + ".nonAlu.vcf", self.rnaEdit.logFile,
                                            self.rnaEdit.textField)
        ...
        if not os.path.isfile(
                self.rnaEdit.params.output + ".noSpliceJunction.vcf") or self.rnaEdit.params.overwrite == True:
            ...
            if not os.path.isfile(self.rnaEdit.params.output + ".noHomo.vcf"):
                nonAluVariants = VariantSet(self.rnaEdit.params.output + ".noSpliceJunction.vcf", self.rnaEdit.logFile,
                                            self.rnaEdit.textField)
        ...
        if not os.path.isfile(self.rnaEdit.params.output + ".noHomo.vcf") or self.rnaEdit.params.overwrite == True:
            ...
            if not os.path.isfile(self.rnaEdit.params.output + ".noBlat.vcf"):
                nonAluVariants = VariantSet(self.rnaEdit.params.output + ".noHomo.vcf", self.rnaEdit.logFile,
                                            self.rnaEdit.textField)
        ...
        if not os.path.isfile(self.rnaEdit.params.output + ".noBlat.vcf") or self.rnaEdit.params.overwrite == True:
            ...
            if not os.path.isfile(self.rnaEdit.params.output + ".editingSites.nonAlu.vcf"):
                nonAluVariants = VariantSet(self.rnaEdit.params.output + ".noBlat.vcf", self.rnaEdit.logFile,
                                            self.rnaEdit.textField)
                # nonAluVariants.deleteNonEditingBases()
                # nonAluVariants.printVariantDict(self.rnaEdit.params.output+".editingSites.nonAlu.vcf")
            else:
                nonAluVariants = VariantSet(self.rnaEdit.params.output + ".editingSites.nonAlu.vcf",
                                            self.rnaEdit.logFile, self.rnaEdit.textField)
        ...
        nonAluVariants.printVariantDict(self.rnaEdit.params.output + ".editingSites.nonAlu.vcf")
        nonAluVariants.printGeneList(self.genome, self.rnaEdit.params.output + ".editingSites.nonAlu.gvf",
                                     printSummary=True)
...
def checkDependencies(args):
    ...
    if not os.path.isfile(args.RefGenome.replace(".fastq", ".dict")):
        ...
        Helper.error("run: 'java -jar %s/picard-tools/CreateSequenceDictionary.jar R=%s  O= %s' to create it" % (
