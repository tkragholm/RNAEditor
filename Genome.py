...
class Genome(object):
    ...
    def __init__(self, gtfFile, logFile=None, textField=0):
        ...
        Helper.info(" [%s] Assembling Genome from %s" % (startTime.strftime("%c"), gtfFile), self.logFile,
                    self.textField)
    ...
    def assembleTranscriptome(self):
        ...
        for uniqGene in self.uniqGeneSet:
            ...
            for transcriptId in self.uniqGene_to_transcriptIds[uniqGene]:
                ...
                codingFrames = array('H', [int(frame) for exonNumber, frame in
                                           sorted(self.transcriptId_to_codingFrames[transcriptId])])
                startCodon = tuple(
                    [interval for exonNumber, interval in sorted(self.transcriptId_to_startCodons[transcriptId])])
                stopCodon = tuple(
                    [interval for exonNumber, interval in sorted(self.transcriptId_to_stopCodons[transcriptId])])

                if len(codingExons) != len(codingFrames):
                    raise Exception("Number of coding Exons and Frames differ for %s %s" % geneId, transcriptId)

                transcript = Transcript(gene, transcriptId, list(transcriptNames), protId, exonIndices,
                                        codingExonIndices, codingFrames, startCodon, stopCodon)
    ...
    def annotateRegion(self, chromosome, start, stop):
        ...
        for gene in self.genesByChromosome[chromosome]:
            ...
            if start < gene.start < stop < gene.end:
                # case 1
                pass
        ...
        for gene in self.genesByChromosome[chromosome]:
            ...
            if gene.start < position < gene.end:
                ...
                if len(gene.codingExons) > 0:
                    ...
                    if (gene.strand and position < gene.codingExons[0][0]) or (
                            not gene.strand and position > gene.codingExons[0][1]):
                        for exon in gene.exons:
                            if exon[0] < position < exon[1]:
                                segment.add("5'UTR")
                                # continue
                    # check if position is behind the last coding exon
                    elif (gene.strand and position > gene.codingExons[-1][1]) or not gene.strand and position < \
                            gene.codingExons[-1][0]:
                        for exon in gene.exons:
                            if exon[0] < position < exon[1]:
                                segment.add("3'UTR")
                                # continue
