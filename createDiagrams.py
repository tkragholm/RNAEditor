...
def createDiagramms(output, geneNumber=20, logFile=None, textField=0):
    ...
    counts2 = Helper.getMMBaseCounts(
        output + ".noBlat.vcf")  # use the var file after all the filters have been applied for nonAlu regions
    ...
    Helper.createBarplot(valueMatrix, fileName, counts1.keys(), ("Alu", "non-Alu"), width=0.4,
                         title="Variants per Base", barText=False, yText="Number", )
    ...
    Helper.createBarplot(valueMatrix, fileName, barNames, ("Alu", "non-Alu"), width=0.4,
                         title="Editing Sites per Position", barText=False, yText="Total Counts")

    valueMatrixPercentage = [Helper.getPercentage(aluPositions), Helper.getPercentage(nonAluPositions)]
    Helper.createBarplot(valueMatrixPercentage, fileNamePercentage, barNames, ("Alu", "non-Alu"), width=0.4,
                         title="Editing Sites per Position", yLim=100, yText="Precentage")
...
parser.add_argument('-o', '--output', metavar='str', type=str,
                    help="output directory and Samplename (Example: '/home/rnaEditor/sample/sample')", required=True)
parser.add_argument('-n', '--geneNumber', metavar='int', type=int, help='Number of genes shown in the figure',
                    default=20)
