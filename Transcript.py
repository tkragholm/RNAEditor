...
class Transcript(object):
    ...
    def __init__(self, gene, transcriptId, names, protId, exonIndices, codingExonIndices,
        ...
        self.startAnalysis = min(
            self.gene.exons[e][0] for e in self.exonIndices) if startAnalysis is None else startAnalysis
        self.stop = max(self.gene.exons[e][1] for e in self.exonIndices) if stop is None else stop
        if codingStart is not None:
            self.codingStart = codingStart
        else:
            self.codingStart = min(self.gene.codingExons[e][0] for e in self.codingExonIndices) if len(
                self.codingExonIndices) else None
        if codingStop != None:
            self.codingStop = codingStop
        else:
            self.codingStop = max(self.gene.codingExons[e][1] for e in self.codingExonIndices) if len(
                self.codingExonIndices) else None
