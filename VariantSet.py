...
class VariantSet(object):
    ...
    def iterator(self, infile):
        while True:
            ...
            variant = Variant(vcfList[0], vcfList[1], vcfList[2], vcfList[3], vcfList[4], vcfList[5], vcfList[6],
                              vcfList[7])
            yield variant
    ...
    def parseVcf(self, vcfFile):
        ...
        Helper.info(" [%s] Parsing Variant Data from %s" % (startTime.strftime("%c"), vcfFile), self.logFile,
                    self.textField)
    ...
    def printVariantDict(self, outfile):
        ...
        if type(outfile) != file:
            raise AttributeError(
                "Invalid outfile type in 'printVariantDict' (need string or file, %s found)" % type(outfile))

        startTime = Helper.getTime()
        Helper.info("[%s] Print Variants to %s" % (startTime.strftime("%c"), outfile.name), self.logFile,
                    self.textField)
        ...
        for v in self.variantDict.values():
            ...
            outfile.write("\t".join(
                [v.chromosome, str(v.position), v.id, v.ref, v.alt, str(v.qual), v.filter, attributeString + "\n"]))
    ...
    def topGenes(self, sumDict, fileName, number=20, value=4):
        if number > len(sumDict):
            if len(sumDict) < 1:
                ...
                return
            Helper.warning("The number of top genes you wanted is bigger than the number of edited genes", self.logFile,
                           self.textField)
            number = len(sumDict)
        if value > 4:
        ...
        Helper.createBarplot(valueMatrix, fileName, barNameTuple, [barName], width=0.35, title="Highly Edited Genes",
                             yLim=yLim, barText=False, yText="Editing Counts")
    ...
    def printGeneList(self, genome, outfile, printSummary=True):
        ...
        startTime = Helper.getTime()
        Helper.info("[%s] Print Genes and Variants to %s" % (startTime.strftime("%c"), outfile.name), self.logFile,
                    self.textField)

        sumFile = open(outfile.name[:outfile.name.rfind(".")] + ".summary", "w")

        outfile.write(
            "\t".join(["#Gene_ID", "Name", "SEGMENT", "#CHROM", "GENE_START", "GENE_STOP", "VAR_ID", "VAR_POS",
            ...
            for a in anno:
                ...
                if gene == "-":
                    out = ["-", "-", ",".join(segments), v.chromosome, "-", "-", v.id, str(v.position),
                           v.ref, v.alt, str(v.qual), "\t".join(v.attributes["BaseCounts"]), totalReads, editedReads,
                           ratio, "\n"]
                    outfile.write("\t".join(out))
                else:
                    out = [gene.geneId, gene.names[0], ",".join(segments), v.chromosome, str(gene.start), str(gene.end),
                           v.id, str(v.position),
                           v.ref, v.alt, str(v.qual), "\t".join(v.attributes["BaseCounts"]), totalReads, editedReads,
                           ratio, "\n"]
                    outfile.write("\t".join(out))
    ...
    def printClusters(self, outFile):
        ...
        startTime = Helper.getTime()
        Helper.info("[%s] Print Clusters to %s" % (startTime.strftime("%c"), outFile.name), self.logFile,
                    self.textField)

        outFile.write("\t".join(
            ["#Chr", "Start", "Stop", "IslandID", "GeneID", "Gene Symbol", "Cluster Length", "Number of Editing_sites",
             "Editing_rate", "\n"]))
        ...
        for cluster in self.clusterDict.keys():
            ...
            outFile.write("\t".join(
                [v.chromosome, str(start), str(end), "Island" + str(cluster),  # Chr","Start","Stop","Cluster Name",
    ...
    def deleteOverlapsFromVcf(self, variants):
        ...
        # get Start time
        startTime = Helper.getTime()
        Helper.info(" [%s] Delete overlapps from %s" % (startTime.strftime("%c"), variantsB.name), self.logFile,
                    self.textField)
    ...
    def getOverlapsFromBed(self, bedFile, getNonOverlaps=False):
        ...
        startTime = Helper.getTime()
        Helper.info("[%s] Delete overlaps from %s" % (startTime.strftime("%c"), bedFile.name), self.logFile,
                    self.textField)
    ...
    def splitByBed(self, bedFile):
        ...
        startTime = Helper.getTime()
        Helper.info("[%s] Split Variants by Bed File %s" % (startTime.strftime("%c"), bedFile.name), self.logFile,
                    self.textField)
        ...
        for index in index_order:
            ...
            while len(candidates) > 0:
                ...
                for c in candidates:
                    ...
                    noise = np.where(labels[c_neighborhood] == -1)[
                        0]  # indexes of candidate neigbours which do not belong to a cluster yet
    ...
    def removeEdgeMismatches(self, bamFile, minDistance, minBaseQual):
        ...
        Helper.info(" [%s] remove Missmatches from the first %s bp from read edges" % (
            startTime.strftime("%c"), str(minDistance)), self.logFile, self.textField)
                ...
                if x.pos == varPos:
                    for pileupread in x.pileups:  # walk through the single reads
                        if not pileupread.is_del and not pileupread.is_refskip:
                            distance = abs(
                                pileupread.alignment.alen - pileupread.query_position) if pileupread.alignment.is_reverse else pileupread.query_position
